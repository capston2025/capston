...
3. **Phase 4 – Autonomous Execution**
   ...
   - `intelligent_orchestrator.py`(GPT-5-mini Vision)가 DOM 스냅샷+스크린샷을 분석해 액션을 결정하고, 에이전트는 LLM이 고른 화면용 `element_id`를 실행 직전에 현재 `snapshot_id + ref_id`로 해석한다.
   - Smart Navigation, Aggressive Text Matching, Semantic Embedding Matching이 결합되어 페이지 이동과 요소 탐색이 자동화된다.
4. **Playwright MCP Host**
   - `phase4/mcp_host.py`(FastAPI + Chromium)가 `browser_snapshot`, `browser_act`(및 legacy `analyze_page` 래퍼)를 제공하며 DOM/스크린샷을 수집하고 `snapshot_id + ref_id`를 생성한다.
...
- **Intelligent Orchestrator (GPT-5-mini Vision)**:
  - DOM+Screenshot 병합 분석으로 다음 액션을 결정한다.
  - LLM 선택 단계는 화면용 `element_id` 기준이고, 실제 실행은 `snapshot_id + ref_id`의 ref-only 계약을 사용한다.
...
- **LLM Vision Client**: GPT-5-mini Vision 호출, DOM 요약 입력(기본 `GAIA_LLM_DOM_LIMIT=260`), ARIA role 확장, 60초 타임아웃.
- **Playwright MCP Host**:
  - FastAPI + Playwright로 Chromium 제어.
  - 에이전트 요청을 받아 실제 DOM/프레임 상태를 수집하고 `snapshot_id`, `ref_id`, `elements_by_ref`를 생성한다.
  - Hash 내비게이션 실패 시 버튼 클릭으로 자동 복구(예: Figma Sites).
  - ARIA role 확장, actionability 계산, ref-only 실행 검증을 담당한다.
