...
class TestGoal(BaseModel):
    ...
    preconditions: List[str] = Field(
        ...
        description="사전 조건 (예: 로그아웃 상태)"
    )

    test_data: Dict[str, Any] = Field(
        default_factory=dict,
        description="테스트에 필요한 데이터 (이메일, 비밀번호 등)"
    )

    success_criteria: List[str] = Field(
        default_factory=list,
        description="성공 조건 (예: 환영 메시지 표시)"
    )

    failure_criteria: List[str] = Field(
        default_factory=list,
        description="실패 조건 (예: 오류 메시지 표시)"
    )

    max_steps: int = Field(
        default=20,
        description="최대 스텝 수 (무한 루프 방지)"
    )

    start_url: Optional[str] = Field(
        default=None,
        description="시작 URL (없으면 현재 페이지에서 시작)"
    )
...
class ActionDecision(BaseModel):
    ...
    element_id: Optional[int] = Field(
        default=None,
        description="대상 요소 ID (click, fill 등에 필요)"
    )
    value: Optional[str] = Field(
        default=None,
        description="입력값 (fill, press 등에 필요)"
    )
    ...
    is_goal_achieved: bool = Field(
        default=False,
        description="목표가 달성되었는지 여부"
    )

    goal_achievement_reason: Optional[str] = Field(
        default=None,
        description="목표 달성 판단 이유 (is_goal_achieved=True일 때)"
    )
...
class GoalTestPlan(BaseModel):
    ...
    goals: List[TestGoal] = Field(
        default_factory=list,
        description="테스트 목표 목록"
    )
