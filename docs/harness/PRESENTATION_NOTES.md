...
이번 구성은 한국 청중에게 익숙한 사이트를 중심으로 잡았다. 기존 공개 suite 11개를 유지하되 모두 사이트당 5개 시나리오로 정규화했고, 신규 19개는 네이버, 다음, 카카오맵, 네이버뉴스, 네이버쇼핑, 쿠팡, 11번가, G마켓, 무신사, 올리브영, YES24, 교보문고, 기상청, 서울 열린데이터광장, 대한민국 구석구석, 정부24, 국가법령정보센터, 멜론, CGV처럼 발표 현장에서 바로 이해 가능한 공개 사이트 위주로 넣었다.
...
| --- | ---: |
| `commerce_product` | 9 |
| `portal_news_community` | 7 |
| `public_data_service` | 5 |
| `developer_tech` | 4 |
| `knowledge_reference` | 3 |
| `finance_game` | 2 |
...
| --- | ---: | --- |
| `stable` | 5 | 문서/레퍼런스 중심, 재현성 baseline |
| `medium` | 11 | 공공/상품/정보 사이트, 일반 공개 웹 수준 |
| `high` | 14 | 포털/뉴스/커머스/커뮤니티처럼 실제 웹 변동성이 큰 구간 |

### 시나리오 템플릿

각 사이트는 아래 5개 흐름으로 정규화한다.
...
| Date | Pack | Sites | Scenarios | Success Rate | Avg Time | Progress Stop | Intervention | Top Failure Reasons | Artifact |
| --- | --- | ---: | ---: | ---: | ---: | ---: | ---: | --- | --- |
| TBD | `external-public-20260507` | 30 | 150 | TBD | TBD | TBD | TBD | TBD | `artifacts/benchmarks/kpi_pack_<timestamp>/summary.md` |
...
- 발표 청중이 바로 이해할 수 있도록 신규 19개 사이트는 한국 포털, 지도, 뉴스, 커머스, 공공데이터, 정부/법령, 음악/영화 사이트 위주로 구성했다.
